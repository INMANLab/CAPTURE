...
p_Welch1 = zeros(length(fRange1),size(np,2));
p_Welch2 = zeros(length(fRange2),size(np,2));
fooofAll1 = zeros(length(fRange2),size(np,2),2);
fooofAll2 = zeros(length(fRange2),size(np,2),2);
for pIdx = 1:size(np,2)
    ...
    p_Welch1(:,pIdx) = pwelch(signal1(:,pIdx), 50, [], fRange1, 250);
    p_Welch1(:,pIdx) = p_Welch1(:,pIdx)/mean(p_Welch1(:,pIdx));
    p_Welch2(:,pIdx) = pwelch(signal2(:,pIdx), 50, [], fRange2, 250);
    p_Welch2(:,pIdx) = p_Welch2(:,pIdx)/mean(p_Welch2(:,pIdx));

    frTemp = fooof(fRange1, p_Welch1(:,pIdx)', [fRange1(1), fRange1(end)], settings, true);
    fooofAll1(:,pIdx,1) = frTemp.fooofed_spectrum';
    fooofAll1(:,pIdx,2) = frTemp.ap_fit';
    frTemp = fooof(fRange2, p_Welch2(:,pIdx)', [fRange2(1), fRange2(end)], settings, true);
    fooofAll2(:,pIdx,1) = frTemp.fooofed_spectrum';
    fooofAll2(:,pIdx,2) = frTemp.ap_fit';
end

pAv_Welch1 = mean(p_Welch1,2);
pAv_Welch2 = mean(p_Welch2,2);
...
% xlabel("log(Freq)")
% title("Second Half")
%% Fit to Wavelet Power
settings.max_n_peaks = 3; %aperiodic component has a better fit if more peaks are removed
settings.aperiodic_mode = 'knee';
% settings.aperiodic_mode = 'fixed';

fooof_results1 = fooof(fRange1, pAv_Wave1', [fRange1(1), fRange1(end)], settings, true);
fooof_results2 = fooof(fRange2, pAv_Wave2', [fRange2(1), fRange2(end)], settings, true);
...
fooof_results1 = fooof(fRange1, pAv_Welch1', [fRange1(1), fRange1(end)], settings, true);
fooof_results2 = fooof(fRange2, pAv_Welch2', [fRange2(1), fRange2(end)], settings, true);
...
% subplot 221
% hold on
% plot(log10(fRange1),squeeze(mean(fooofAll1(:,:,1),2))-squeeze(mean(fooofAll1(:,:,2),2)))
% plot(log10(fRange2),squeeze(mean(fooofAll2(:,:,1),2))-squeeze(mean(fooofAll2(:,:,2),2)))
subplot 221
hold on
plot(fRange1,squeeze(mean(fooofAll1(:,:,1)-fooofAll1(:,:,2),2)))
plot(fRange2,squeeze(mean(fooofAll2(:,:,1)-fooofAll2(:,:,2),2)))
ylabel("Fit on Each Epoch")
subplot 222
hold on
plot(fRange1,squeeze(mean(fooofAll1(:,:,2),2)))
plot(fRange2,squeeze(mean(fooofAll2(:,:,2),2)))
subplot 223
hold on
plot(fRange1,fooof_results1.fooofed_spectrum-fooof_results1.ap_fit)
plot(fRange2,fooof_results2.fooofed_spectrum-fooof_results2.ap_fit)
ylabel("Fit on Average")
subplot 224
hold on
plot(fRange1,fooof_results1.ap_fit)
plot(fRange2,fooof_results2.ap_fit)
title("Fixed")
%%
a=0;

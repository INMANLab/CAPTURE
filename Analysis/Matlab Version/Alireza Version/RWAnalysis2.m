classdef RWAnalysis2 < handle
    ...
    % *fooof and pepisode added to 2sec full walk table
    % add in/out and conf matrix plots to gui
    ...
    % calc velocity change by patient instead of entire group
    ...
    methods %initialization, calculations, etc
        ...
        function obj = RWAnalysis2(varargin) %constructor (runs when object is created for the first time)
            ...
            obj.RootDir = 'D:\CAPTURE Project\GUI\Data\';
                        ...
                        if isfield(SS,'ntp_bio')
                            ...
                            if ~isempty(SS.ntp_bio)
                                ...
                                for m=1:length(pks) %going highest to lowest
                                    ...
                                    % if m<floor(length(pks)/3)
                                    %     evnt_table = vertcat(evnt_table,table({"EDAPeakHighTercile"},{""},pupilframe,goproframe,npsample,ntp_bio,'VariableNames',{'Event','Description','PupilFrame','GoProFrame','NPSample','NTP'}));
                                    % elseif m>=floor(length(pks)/3) && m<2*floor(length(pks)/3)
                                    %     evnt_table = vertcat(evnt_table,table({"EDAPeakMidTercile"},{""},pupilframe,goproframe,npsample,ntp_bio,'VariableNames',{'Event','Description','PupilFrame','GoProFrame','NPSample','NTP'}));
                                    % else
                                    %     evnt_table = vertcat(evnt_table,table({"EDAPeakLowTercile"},{""},pupilframe,goproframe,npsample,ntp_bio,'VariableNames',{'Event','Description','PupilFrame','GoProFrame','NPSample','NTP'}));
                                    % end
                                end
                                for m=1:length(trs)
                                    if trs(m)<pks5
            ...
            for k=1:length(obj.PatientList)
                ...
                for m=1:4 %by channel
                    ...
                    segtable = table(...
                        ...
                        ds_xs_norm,ds_gz_norm,ds_am_norm,ds_kd_norm,ds_ed_norm,...
        ...
        function filterMultTransData(obj,varargin)
            ...
            MT = table(pt',wk',swk',gwk',rg',lb',ch',np_idx',ns',ev',ds',ds_idx',...
                xs_idx',xs_vchg',gz_idx',am_idx',kd_idx',wv_idx',bi_idx',pe_idx',...
                'VariableNames',{'patient','walk','stopwalk','gowalk','regionnum',...
                'regionlabel','chan','npidx','nsamp','evnt','desc','descidx',...
                'xsidx','xsvchg','gzidx','amidx','kdidx','wvidx','biidx','peidx'});
        ...
        function getFilteredMultTransData(obj,transrng,varargin)
            ...
            for k=1:size(uPtWkCh,1)
                ...
                if ~isempty(obj.MultTable{pt}.d_bio{wk})
                    ...
                    d_ed(idx(~ridx),:,:) = d; %trial x time x freq
                end
        ...
        function copyData(obj,varargin)
            ...
            for m=1:length(obj.PatientList) %iterate patient
                ...
                for k=1:totalwalks
                    ...
                    dst = fullfile('D:\CAPTURE Project\GUI\Data\',fname);
    ...
    methods %general plotting
        ...
        function varargout = plotMultTransSpecGramPerm(obj,varargin)
            ...
            predlist = [{'xs','gz','kd','am','ed','pe1','pe2','pe3'};{'Vel','Fix','KDE','Amb','Eda','PeT','PeA','PeB'}];
...
%             sPM = std(PM,0,3); %std across permutations
            ...
            yyaxis(aH,'right');
            for k=1:size(predlist,2) %xs,gz,kd,am,ed,pe1,pe2,pe3
                dx = obj.MultTrans.MTd.(['d_',predlist{1,k}]);
                tx = obj.MultTrans.MTd.(['tsec_',predlist{1,k}]);
                if contains(predlist{1,k},'pe')
                    dxx = dx;
                else
...
%             end
        ...
        function plotMultInOutFreq(obj,varargin)
            ...
            %
            %plotMultInOutFreq('ftype','Theta','saveflag',true); %Theta, Alpha, Beta, Gamma, HG (wv or mt prefix for wavelet or multitaper)
            p = obj.parseInputs(varargin{:});
            if isempty(obj.MultSeg)
                obj.getMultSegData;
            end
            if isempty(p.ftype)
                error('ftype is missing!')
            end
            ...
            fidx = find(contains(flbls,regexprep(p.ftype,'^(wv|mt)','')),1);
            if isempty(fidx)
                error('ftype was incorrect!');
            end
            frng = flist(fidx,:);
            fstr = p.ftype; %add 'n' for log10 normalized power (median-normalized by freq across walk, then mean across band, median across 2sec segment, and log10 applied -> multiply by 10 to get dB)
            ...
            for k=1:size(obj.MultSeg.uPtChan,1)
                ...
                pwr_rng = SegTable.(fstr)(idx); %log10 applied if [n]Theta, etc.
                inflag = SegTable.InFlag(idx);

                pwr_rng_in = pwr_rng(inflag);
                pwr_rng_out = pwr_rng(~inflag);

                in_n = sum(~isnan(pwr_rng_in));
                out_n = sum(~isnan(pwr_rng_out));

                in_pwr_rng = median(pwr_rng_in,'omitnan');
                out_pwr_rng = median(pwr_rng_out,'omitnan');
            ...
            ss = [SS.(['In_',fstr]),SS.(['Out_',fstr])].*10; %log10 already applied to median normalized (across walk) power
            ...
            InOutTable = removevars(SS,{['In_',fstr],['Out_',fstr]});
...
%             [c,m] = multcompare(stats,'alpha',0.05,'display','on');
        ...
        function plotMultInOutPred(obj,varargin)
            ...
            %
            %plotMultInOutPred('predtype','Vel','saveflag',true); %[n]Vel, [n]Fix, [n]KDE, [n]Amb (this is the only one that is log10 normalized), [n]EDA
            p = obj.parseInputs(varargin{:});
            if isempty(obj.MultSeg)
                obj.getMultSegData;
            end
...
%             [c,m] = multcompare(stats,'alpha',0.05,'display','on');
        ...
        function varargout = plotMultGLME(obj,varargin)
            ...
            pnames = strtrim(regexp(glmemdl,'\+','split')); pnames(1) = [];
